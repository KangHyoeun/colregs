...
def calculate_cpa_tcpa(
    ...
    Returns:
        ...
        tcpa: Time to CPA (seconds), 음수는 이미 CPA 통과
    ...
    if rel_speed_sq < 1e-6:
        ...
        return (current_distance, np.inf)
    ...
    cpa_dx = dx + dvx * tcpa
    cpa_dy = dy + dvy * tcpa
    dcpa = np.sqrt(cpa_dx**2 + cpa_dy**2)
...
def calculate_miss_distance(
    ...
    if tcpa > time_horizon:
        ...
        dx = (ts_position[0] + ts_velocity[0] * time_horizon) - \
             (os_position[0] + os_velocity[0] * time_horizon)
        dy = (ts_position[1] + ts_velocity[1] * time_horizon) - \
             (os_position[1] + os_velocity[1] * time_horizon)
        return np.sqrt(dx**2 + dy**2)
