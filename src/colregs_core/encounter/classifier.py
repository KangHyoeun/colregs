...
class EncounterClassifier:
    ...
    def classify(
        ...
        # 상대 침로 계산
        relative_course = normalize_angle(ts_heading - os_heading)
    ...
    def _classify_encounter_type(
        ...
        # Rule 15: Crossing
        if self._is_crossing_give_way(relative_bearing):
            return EncounterType.CROSSING_GIVE_WAY
        
        if self._is_crossing_stand_on(relative_bearing):
    ...
    def _is_overtaking(
        ...
        조건:
        1. TS가 OS 선미 섹터(112.5° - 247.5°)에 위치
        2. OS가 TS보다 빠른 속도
        
        Notes:
            COLREGs Rule 13에서 overtaking은 TS의 선미 22.5° 부채꼴 밖에서
            접근하는 것으로 정의. 실무에서는 TS가 OS 선미 섹터에 있으면
            OS가 overtaking vessel로 간주.
        """
        # TS가 선미 섹터에 있는지
        ...
        in_stern_sector = overtaking_start <= relative_bearing <= overtaking_end
        
        if not in_stern_sector:
            return False
        
        # Aspect angle 확인: OS가 TS의 선미 섹터에서 접근
        # aspect_angle이 대략 135° - 225° 범위면 OS가 TS 뒤에서 접근
        # 또는 정확히 반대편(180° 근처)이면 같은 방향으로 항해 중
        os_from_stern = (135 <= aspect_angle <= 225) or abs(aspect_angle - 180) < 30
        
        # COLREGs에서 overtaking은 추월하는 선박이 피항 책임
        # OS가 더 빠르면 추월 상황
        speed_check = os_speed > ts_speed * 0.9
        
        return in_stern_sector and speed_check
    ...
    def _is_crossing_give_way(self, relative_bearing: float) -> bool:
        """
        Rule 15: Crossing - OS가 Give-way Vessel
        
        조건: TS가 OS의 우현(5° - 112.5°)에 위치
        """
        return self.BOW_CROSSING_START < relative_bearing < self.BOW_CROSSING_END
    
    def _is_crossing_stand_on(self, relative_bearing: float) -> bool:
        """
        Rule 15: Crossing - OS가 Stand-on Vessel
        
        조건: TS가 OS의 좌현(247.5° - 355°)에 위치
        """
        return self.STERN_CROSSING_START < relative_bearing < self.STERN_CROSSING_END
